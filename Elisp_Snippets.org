...
variable. Before run add the code below when the user run 
=M-x async-shell-command= scala it shows an error message: 
...
In this section is supposed that the org-buffer has this code block:
